...
def parse() -> dict:
    ...
    parser.add_argument('--config_path',
                        # required=True,
                        default='./configs/downstream/st_mem_super.yaml',
                        type=str,
                        metavar='FILE',
...
def main(config):
    ...
    print(f'val dataset len: {len(dataset_valid)}') ##
    ...
    #### 250108 추가 #####
    # learnable 파라미터 개수 확인
    print('[model]')
    print(model_without_ddp)
    sum_params = sum(param.numel() for param in model_without_ddp.parameters() if param.requires_grad)
    print(f'[Number of learnable parameter]: {sum_params:,}')
    #### 250108 추가 #####
    ...
    if config.get('test', False) and misc.is_main_process():
        ...
        print(f'test dataset len: {len(dataset_test)}') ##
